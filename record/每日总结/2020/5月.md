...
2.  利用 [elementFromPoint](https://developer.mozilla.org/zh-CN/docs/Web/API/Document/elementFromPoint) 写了一个 [去除顶部遮挡](https://greasyfork.org/zh-CN/scripts/403713-%E5%8E%BB%E9%99%A4%E9%A1%B6%E9%83%A8%E9%81%AE%E6%8C%A1) 的油猴脚本
    ...
    **效果** :
    ...
    <iframe src="//player.bilibili.com/player.html?aid=838226532&bvid=BV1xg4y1B7B7&cid=192784553&page=1" scrolling="no" border="0" frameborder="no" framespacing="0" allowfullscreen="true"> </iframe>

## 2020-5-21

1.  一个让返回值是 promise 的函数顺序执行的函数
...
3. 本地以图搜图工具 [嗅图狗](https://blog.csdn.net/sinat_27382047/article/details/83040411)

## 2020-5-24

1. 今天起的比较早，打算把我的博客改成 svelte 的 sapper 来提供 ssr 渲染，结果浪费了挺多时间的。首先是 svelte 的 ts 提示还不太好，然后找的几个 sapper typescript 模板只有一个我跑起来了( [github 地址](https://github.com/elcobvg/sapper-typescript-rollup)) 跑完后发现页面中路径跳转不对，全都跳根路径去了。幸好记忆中有关于 base 标签的映像，排查这里花了一些时间但不是太多。这个问题的缘由在 [这里](https://github.com/sveltejs/sapper/issues/904) 我也在下面提了一个解决方案
...
今日翻看今年五月三日游苏州穹窿山拍的照片了解了一下花朝节 ![眉眼](./眉眼.jpg)
