...
export function run() {
	...
	/** 高亮代码块 */
	const code_el = document.querySelectorAll(`[class*="lang"]`);
	if (code_el === null) throw '未找到code块';
	const code = Array.from(code_el);
	const div_list: [/** 放代码的div */ HTMLElement, /** 放编辑器的div */ HTMLElement][] = code.map(
		function (el) {
    ...
    `;
			...
			el[editorKey]['resolve_cb'] = resolve_cb;
	...
	if (show_editor.length) {
		// require.config({
		// 	paths: {
		// 		vs: /** cdn 地址 */ 'https://cdnjs.cloudflare.com/ajax/libs/monaco-editor/0.19.2/min/vs'
		// 	}
		// });
		// //@ts-ignore
		...
		// 		});
		// 		el[editorKey].resolve_cb({ editor, monaco });
